...
# Override ENTRYPOINT so Railway/run args don't get passed to authorizer (avoids "unknown flag: -c").
# v2 uses CLI arguments only. Railway injects env vars; shell expands them into CLI flags.
ENTRYPOINT ["/bin/sh", "-c"]
CMD ["./authorizer \
